...
import { Client as iDriveClient } from 'idrive';
...
export default class iDrive implements IStorageAdapterV2 {
  private idriveClient: iDriveClient;
  private input: any;
  ...
  public async fileRead(key: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.idriveClient.getObject(this.input.bucket, key, (err, data) => {
        if (err) {
          return reject(err);
        }
        if (!data) {
  ...
  public async init(): Promise<any> {
    ...
    this.idriveClient = new iDriveClient(this.input);
  }
